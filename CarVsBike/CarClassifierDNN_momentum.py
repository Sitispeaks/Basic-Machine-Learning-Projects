...
def gradDescent(parameters, v, learning_rate):
	L = len(parameters) // 2 
...
def momentum_init(parameters):
	...
	return v
def momentum_update(v,grads,i,beta=0.9):
	L = len(v) // 2
	for l in range(L):
...
def model(X, Y, layer_dims, learning_rate = 0.01, iterations = 200, print_cost = False):	
	...
	for i in range(iterations):
		...
		v = momentum_update(v,grads,i,beta=0.9)
		parameters = gradDescent(parameters, v, learning_rate)
...
parameters = model(X_train, Y_train, layer_dims,0.01,4000,True)
